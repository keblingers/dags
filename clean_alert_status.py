...
def dag_previous_run(prev_start_date_success):
    if prev_start_date_success is not None:
        ...
        query = f"ini if : select notification_status from notification where notification_date between '{last_run}' and '{now_time}' and notification_status = 'ERROR' "
    else:
        satu_jam = now - timedelta(hours=1)
        format_satu_jam = satu_jam.strftime('%Y-%m-%d %H:%M:%S')
        query = f"ini else : select notification_status from notification where notification_date between '{format_satu_jam}' and '{now_time}' and notification_status = 'ERROR' "
...
with DAG('clean_alert_status',
    ...
    get_prev_run = PythonOperator(#BranchPythonOperator(
